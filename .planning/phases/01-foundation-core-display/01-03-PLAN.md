...
files_modified:
  ...
  - src/data/project.rs
  - src/data/roadmap.rs
  - src/data/planning_state.rs
  - src/data/parser.rs
autonomous: true

must_haves:
  truths:
    - "Application parses PROJECT.md into Project struct"
    - "Application parses ROADMAP.md into phases with requirements"
    - "Application parses STATE.md into current project state"
    - "Parser handles missing files gracefully"
  artifacts:
    - path: "src/data/project.rs"
      provides: "Project struct parsed from PROJECT.md"
      exports: ["Project"]
    - path: "src/data/roadmap.rs"
      provides: "Roadmap, Phase, Requirement structs from ROADMAP.md"
      exports: ["Roadmap", "Phase", "Requirement", "PhaseStatus", "RequirementStatus"]
    - path: "src/data/planning_state.rs"
      provides: "PlanningState struct from STATE.md"
      exports: ["PlanningState"]
    - path: "src/data/parser.rs"
      provides: "Markdown parsing functions for .planning/ files"
      exports: ["load_planning_data", "PlanningData"]
  key_links:
    - from: "src/data/parser.rs"
      to: "pulldown_cmark"
      via: "markdown event stream parsing"
      pattern: "pulldown_cmark::Parser"
    - from: "src/data/parser.rs"
      to: "src/data/*.rs"
      via: "constructs domain types from parsed markdown"
      pattern: "Project|Roadmap|PlanningState"
---

<objective>
Create data models and markdown parser for .planning/ files.

Purpose: The TUI needs to display structured data from PROJECT.md, ROADMAP.md, and STATE.md. This plan creates the data layer that transforms markdown into typed Rust structs.

Output: Data models (Project, Roadmap, Phase, Requirement, PlanningState) and a parser that loads them from .planning/ directory.
</objective>
...
<task type="auto">
  <name>Task 1: Create data model types</name>
  <files>
    src/data/mod.rs
    src/data/project.rs
    src/data/roadmap.rs
  ...
  <action>
Create src/data/ directory and data model files.
...
Verify all types are public and re-exported from data/mod.rs.
  </verify>
  <done>
Data models defined: Project, Roadmap, Phase, Requirement, PlanningState with appropriate fields and status enums.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create markdown parser</name>
  <files>src/data/parser.rs</files>
  <action>
Create src/data/parser.rs that parses .planning/ files using pulldown-cmark:
...
fn load_roadmap(path: &Path) -> Result<Roadmap> {
    ...
    for line in content.lines() {
        ...
        // Overview section
        else if trimmed.starts_with("## Overview") {
            // Next non-empty lines until next heading are overview
        }
    }
...
fn load_state(path: &Path) -> Result<PlanningState> {
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with("**Phase:**") {
            ...
            state.current_phase_name = parts.get(1).map(|s| s.trim().to_string()).unwrap_or_default();
        }
        // **Plan:** value
        ...
        // **Status:** value
            ...
            state.status = trimmed.strip_prefix("**Status:**").unwrap_or("").trim().to_string();
        }
        ...
        else if trimmed.starts_with("| v1 Requirements |") || trimmed.starts_with("| Total Requirements |") {
            if let Some(value) = extract_table_value(trimmed) {
...
mod tests {
    ...
    fn test_load_project_basic() {
        ...
        std::fs::write(&project_path, "# Test Project\n\n## Core Value\n\nA test project.\n").unwrap();
    ...
    fn test_load_roadmap_phases() {
        ...
        std::fs::write(&roadmap_path, r#"# Roadmap
...
"#).unwrap();
    ...
    fn test_load_state_metrics() {
        ...
        std::fs::write(&state_path, r#"# Project State
...
```
  ...
  <verify>
Run `cargo build` - should compile.
Run `cargo test` - all parser tests should pass.
Test against actual .planning/ files:
```bash
# In a test, load_planning_data from the actual project .planning/
```
  </verify>
  <done>
Parser loads PROJECT.md, ROADMAP.md, STATE.md into typed structs. Handles missing files gracefully. Unit tests pass.
  </done>
</task>
...
2. `cargo test` passes all parser tests
3. Parser correctly extracts:
   - Project name and description from PROJECT.md
   - Phase numbers, names, goals, requirements from ROADMAP.md
   - Current phase, status, metrics from STATE.md
4. Missing files return default values (no crashes)
</verification>

<success_criteria>
- Data models match research spec (Project, Roadmap, Phase, Requirement, PlanningState)
- Parser uses pulldown-cmark for markdown parsing
- Parser handles malformed or missing files gracefully
- Unit tests verify parsing against sample content
- Types are re-exported from data/mod.rs for clean imports
</success_criteria>
